...
const inBucket = async (area, v) => {
  ...
  try {
    ...
    // Name, code and HTTP status only. They say what is wrong — InvalidAccessKeyId,
    // AccessDenied, ENOTFOUND, ECONNREFUSED — without repeating the endpoint or
    // bucket back into a log this public repository publishes.
    const bits = [e?.name, e?.code, e?.$metadata?.httpStatusCode].filter(Boolean);
    return { error: bits.length ? bits.join("/") : "unknown" };
  }
};
